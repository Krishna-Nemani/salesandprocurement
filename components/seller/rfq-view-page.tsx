...
interface SellerRFQViewPageProps {
  rfq: RFQ & {
    items: RFQItem[];
    buyerCompany?: {
      id: string;
...
export function SellerRFQViewPage({ rfq }: SellerRFQViewPageProps) {
  const router = useRouter();
  const [isProcessing, setIsProcessing] = useState(false);


  const handleCreateQuotation = () => {
    router.push(`/seller/quotations/new?rfqId=${rfq.id}`);
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(`/api/seller/rfqs/${rfq.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to approve RFQ");
      }
      ...
      alert(error instanceof Error ? error.message : "Failed to approve RFQ");
    } finally {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to decline RFQ");
      }

      alert("RFQ declined successfully!");
      router.refresh();
    } catch (error) {
      console.error("Error declining RFQ:", error);
      alert(error instanceof Error ? error.message : "Failed to decline RFQ");
    } finally {
      setIsProcessing(false);
    }
  };

  const canApproveOrDecline = rfq.status === RFQStatus.PENDING || rfq.status === RFQStatus.DRAFT;
  const canCreateQuotation = rfq.status === RFQStatus.PENDING || rfq.status === RFQStatus.DRAFT || rfq.status === RFQStatus.APPROVED;
    ...
    <div className="space-y-6 print:space-y-4">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push("/seller/rfqs")}>
        ...
        <div className="flex items-center gap-2">
          ...
          <DocumentViewActions
            pdfUrl={`/api/pdf/seller/rfq/${rfq.id}`}
