...
export function SellerQuotationListClient() {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const response = await fetch(`/api/seller/quotations?${params.toString()}`);
      if (!response.ok) {
  ...
  // Filter quotations by search query (client-side search)
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((quotation) => {
        return (
          ...
          quotation.buyerCompanyName.toLowerCase().includes(query) ||
  ...
  const stats = useMemo(() => {
    return {
      ...
      accepted: quotations.filter((q) => q.status === QuotationStatus.ACCEPTED || q.status === QuotationStatus.APPROVED).length,
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/seller/quotations/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4 flex items-center justify-between">
        <p className="text-purple-900 dark:text-purple-100">
          Ready to send a new offer? Create a new quotation now!
        </p>
        <Button
          onClick={() => router.push("/seller/quotations/new")}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border">
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Overview of all sent quotations.
          </p>
        </div>
              ...
              {filteredQuotations.length === 0 ? (
                ...
                filteredQuotations.map((quotation) => (
                  <TableRow key={quotation.id}>
                    ...
                    <TableCell>
                      {quotation.rfq?.rfqId || quotation.rfqId || "-"}
                    </TableCell>
                    <TableCell>{quotation.buyerCompanyName}</TableCell>
                    <TableCell>{formatDate(quotation.quoteDateIssued)}</TableCell>
                      ...
                      >
                        <DropdownMenuItem
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            window.location.href = `/seller/quotations/${quotation.id}`;
                          }}
                        >
                        ...
                        <DropdownMenuItem
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            router.push(`/seller/quotations/${quotation.id}/edit`);
                          }}
                        >
